...
if "messages" not in st.session_state:
    # Guardamos las instrucciones del sistema en una variable separada de la sesión
    st.session_state.messages = []
    
    # Forzamos la primera llamada simulando el inicio para que el agente se presente
    with st.spinner("Iniciando asistente administrativo..."):
        try:
            response = co.chat(
                model="command-r-08-2024",
                messages=[
                    {"role": "system", "content": SYSTEM_PROMPT},
                    {"role": "user", "content": "Hola. Por favor, preséntate y solicita el primer bloque de información para los certificados."}
                ]
            )
            # Guardamos la amable presentación del asistente en nuestro historial visible
...
# Entrada de texto del usuario (El input abajo del chat)
if user_input := st.chat_input("Escribe aquí tu respuesta..."):
    ...
    # 2. Llamar a la API enviando las instrucciones del sistema + todo el historial acumulado
    with chat_container:
        with st.chat_message("assistant", avatar="🤖"):
            message_placeholder = st.empty()
            with st.spinner("Pensando..."):
                try:
                    # Construimos el envío incluyendo siempre el SYSTEM_PROMPT al inicio
